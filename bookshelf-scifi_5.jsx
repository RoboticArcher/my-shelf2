...
{
  "taste_profile": "2-sentence synthesis of their reading taste written directly to them (use 'you' and 'your')",
  "recommendations": [
      ...
      <div className="modal modal-wide">
        <div className="modal-header">
          <div>
            ...
            <div className="modal-sub">AI-Reasoned · Based on Your Actual Taste</div>
          </div>
          <button className="close-btn" onClick={onClose}>×</button>
        </div>
        ...
        {error && (
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 12, color: "var(--ink3)", lineHeight: 1.8, marginBottom: 12 }}>
              No API key? Copy the prompt below and paste it into <a href="https://claude.ai" target="_blank" rel="noreferrer" style={{ color: "var(--cyan)" }}>claude.ai</a> instead — it's free.
            </div>
            <button className="btn-ghost" style={{ width: "100%", padding: "10px 0" }} onClick={copyPrompt}>
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              { type: "text", text: `Look at this bookshelf photo. Identify every book you can read from the spines. For each book, provide the title and author. Respond ONLY with valid JSON, no markdown:\n{\n  "books": [\n    { "title": "...", "author": "..." }\n  ]\n}` }
            ]
          }]
        })
      });
  ...
  return (
    <div className="backdrop" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal modal-wide">
        <div className="modal-header">
          <div>
            <div className="modal-title">Scan Bookshelf</div>
            <div className="modal-sub">Photo → AI reads spines → Books added instantly</div>
          </div>
          <button className="close-btn" onClick={onClose}>×</button>
        </div>
        ...
        {stage === "upload" && (
          <>
            <div
              ...
              style={{
                border: "2px dashed var(--border2)", borderRadius: 10, padding: "40px 20px",
                textAlign: "center", cursor: "pointer", transition: "border-color 0.15s, background 0.15s",
                background: preview ? "var(--bg)" : "transparent", marginBottom: 16,
              }}
            ...
            <button className="btn-primary" style={{ width: "100%", padding: "12px 0", fontSize: 13, opacity: imageData ? 1 : 0.4, cursor: imageData ? "pointer" : "not-allowed" }}
              onClick={() => imageData && scan()}>
              Scan with AI
            </button>
          </>
        )}
        ...
        {stage === "review" && (
          <>
            <div style={{ fontSize: 12, color: "var(--ink3)", marginBottom: 16 }}>
              Found <strong>{found.length} books</strong>. Uncheck any that are wrong, then add them all at once. You can rate them after.
            </div>
            <div style={{ maxHeight: 300, overflowY: "auto", marginBottom: 20, display: "flex", flexDirection: "column", gap: 8 }}>
              {found.map((b, i) => (
                <div key={i} onClick={() => toggle(i)} style={{
                  display: "flex", alignItems: "center", gap: 12, padding: "10px 14px",
                  background: b.include ? "var(--cyan-dim)" : "var(--bg)",
                  border: `1.5px solid ${b.include ? "var(--cyan-mid)" : "var(--border)"}`,
                  borderRadius: 6, cursor: "pointer", transition: "all 0.15s",
                }}>
                  <div style={{ width: 18, height: 18, borderRadius: 4, border: `2px solid ${b.include ? "var(--cyan)" : "var(--border2)"}`, background: b.include ? "var(--cyan)" : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all 0.15s" }}>
            ...
            <button className="btn-primary" style={{ width: "100%", padding: "12px 0", fontSize: 13 }} onClick={addAll}>
              Add {found.filter(b => b.include).length} Books to Shelf
            </button>
            <button className="btn-ghost" style={{ width: "100%", marginTop: 8, padding: "10px 0" }} onClick={() => setStage("upload")}>
